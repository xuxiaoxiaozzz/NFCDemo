...
@interface NFCManager ()<NFCNDEFReaderSessionDelegate,NFCTagReaderSessionDelegate>{
    BOOL isReading;
}
...
@property(nonatomic,weak)id<NFCTag> currentTag;
@end
                    ...
                    [tag readNDEFWithCompletionHandler:^(NFCNDEFMessage * _Nullable message, NSError * _Nullable error) {
                        if (error) {
                            ...
                            if (self.scanSuccessBlock) {
                                ...
                                self.scanSuccessBlock(message,mifareTag.identifier);
                            }
                        }
                    }];
                }
        ...
        if (self->isReading) {
            if (@available(iOS 11.0, *)) {
                for (NFCNDEFMessage *message in messages) {
                    ...
                    [self invalidateSession];
                    if (self.scanSuccessBlock) {
                        self.scanSuccessBlock(message,nil);
                    }
                }
            }
        }
